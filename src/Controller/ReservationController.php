...
class ReservationController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $em, int $id): Response
    {
        ...
        // Pre-fill all non-form fields BEFORE form creation so auto-mapped NotNull passes
        ...
        // Recompute real prixTotal from bound form data
        if ($form->isSubmitted()) {
            $dateDebut = $reservation->getDateDebut();
            $dateFin   = $reservation->getDateFin();
            $personnes = $reservation->getNombrePersonnes();
            $prix      = $logement->getPrix() ?? 0;
            if ($dateDebut && $dateFin && $personnes && $prix) {
                $nbNuits = $dateDebut->diff($dateFin)->days;
                $reservation->setPrixTotal((float)($nbNuits * $prix * $personnes));
            }
        }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($reservation->getDateDebut() < $today) {
                $this->addFlash('error', 'La date d\'arrivée ne peut pas être dans le passé.');
            ...
            $conflits = $em->getRepository(Reservation::class)->createQueryBuilder('r')
                ...
                ->setParameter('cancelled', ['ANNULÉE', 'ANNULEE', 'annulee', 'refusee'])
    ...
    public function list(
        ...
        try {
            ...
            foreach ($reservations as $r) {
                if (!$r->getDateDebut()) { $alertes24h[$r->getId()] = ['niveau' => 'ok', 'message' => null, 'peut_modifier' => true]; continue; }
        ...
        $analysesPrix = [];
        foreach ($reservations as $r) {
            if (!$r->getLogement()) { $analysesPrix[$r->getId()] = ['badge' => 'info', 'message' => 'N/A', 'diff_pct' => 0]; continue; }
            try {
        ...
        // ── 3. Dates Optimales ──
        $datesExistantes = array_values(array_filter(array_map(
    ...
    // ── API: blocked date ranges for a logement ──
    #[Route('/api/blocked/{logementId}', name: 'api_blocked_dates', methods: ['GET'])]
    public function blockedDates(int $logementId, Request $request, EntityManagerInterface $em): JsonResponse
    {
        $excludeId    = (int) $request->query->get('exclude', 0);
        $reservations = $em->getRepository(Reservation::class)->findBy(['logement' => $logementId]);
        $cancelled    = ['annulée', 'ANNULEE', 'annulee', 'refusee', 'rejetee'];

        $blocked = [];
        foreach ($reservations as $r) {
            if ($r->getId() === $excludeId) continue;
            if (in_array(strtolower($r->getStatus()), $cancelled)) continue;
            $blocked[] = [
        ...
        return new JsonResponse($blocked);
    }
